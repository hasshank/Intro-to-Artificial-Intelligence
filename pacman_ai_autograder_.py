...
def readCommand(argv):
  ...
  parser.add_option('--print-test-case',
                    dest='printTestCase',
                    action = 'store_true',
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap = ERROR_HINT_MAP,
  ...
  for q in test_subdirs:
    ...
    if not os.path.join(testRoot, q)
    ...
    for t in sorted(tests):
      ...
      testClass = getattr(projectTestClasses, testDict['class'])
      testCase = testClass(question, testDict)
      def makefun(testCasen, solution_file):
  ...
  grades.grade(sys.module[__name__], bonusPic = projectParams.BONUS_PIC)
...
def getDisplay(graphicsByDefault, options = None):
  ...
  if graphics:
    try:
      ...
      pass
  import textDisplay
  return testDisplay.NullGraphics()
...
def generatePublicTests(moduleDict, privateRoot = 'private_test_cases', publicRoot='test_cases'):
  ...
  for q in sorted(os.listdir(privateRoot)):
    ...
    private_subdir_path = os.path.join(publicRoot, q)
    ...
    questionDict = testParser.TestParser(os.path.join(private_subdir_path, 'CONFIG')).parse()
    questionClass = getattr(testClasses, questionDict['class'])
